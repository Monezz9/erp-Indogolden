            ...
            <table class="min-w-[1280px] w-full text-sm">
                ...
                <tbody>
                    @forelse($this->rows() as $row)
                        @php
                            $draftReceipt = $row->goodsReceipts->first(fn ($receipt) => $receipt->status === \App\Enums\GoodsReceiptStatus::Draft);
                            $canCreateReceipt = $this->canCreateReceipt($row);
                            $orderedQty = $row->items->sum('ordered_qty');
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800">
                            ...
                            <td class="px-3 py-2 text-right">{{ \App\Support\IndoNumber::decimal($orderedQty) }}</td>
                            ...
                            <td class="px-3 py-2 text-right">
                                ...
                                @if($draftReceipt)
                                    <button type="button" wire:click="confirmReceipt({{ $draftReceipt->id }})" class="rounded-lg bg-red-600 px-3 py-1 text-xs font-medium text-white hover:bg-red-700">Konfirmasi Terima</button>
                                @elseif($canCreateReceipt)
                                    <button type="button" wire:click="createReceipt({{ $row->id }})" class="rounded-lg bg-red-600 px-3 py-1 text-xs font-medium text-white hover:bg-red-700">Buat Penerimaan</button>
                                @endif
                            </td>
                        </tr>
